    ...
    function qpbGeometryToGeoJson(feature, layer, table, nativeGeometryExpression) {
        ...
        try {
            ...
            if (typeof qpbReportGeometryEvaluator !== "undefined") {
                ...
                // Emit JSON natively so a map is not silently converted to an empty string.
                if (typeof evaluated === "string" && evaluated.length <= 2 * qpbReportFullGeometryMaxBytes) {
                    evaluated = evaluated ? JSON.parse(evaluated) : null;
                }
...
__QPB_MAP_FEATURE_DISPATCH__
    ...
    function qpbBuildAnalytics(datasets, joined) {
        ...
        var communities = datasets.community || [];
        var coverTotals = {}, coverCounts = {}, coverLabels = {}, invalidCover = 0;
        ...
        for (var chartObservationIndex = 0; chartObservationIndex < chartObservations.length; chartObservationIndex++) {
            ...
            coverLabels[chartSpecies] = {
                korean:String(chartObservationAttrs.selected_korean_name || "").trim(),
                scientific:String(chartObservationAttrs.selected_scientific_name || "").trim()};
        ...
        for (var coverSpecies in coverTotals) {
            coverStats.push({name:coverSpecies, korean:coverLabels[coverSpecies].korean,
                scientific:coverLabels[coverSpecies].scientific,
                value:coverTotals[coverSpecies] / coverCounts[coverSpecies]});
        }
...
function qpbSpeciesChartLabel(species){return String(species.korean||"").trim()||String(species.scientific||"").trim()||"미동정";}
function qpbCoverChartValues(){return (((data.summary_stats||{}).chart_stats||{}).cover||[]).map(function(item){var legacy=String(item.name||"").split(" · ");return {key:item.name,name:qpbSpeciesChartLabel({korean:item.korean==="국명 미입력"?"":item.korean||legacy[0],scientific:item.scientific||legacy[1]}),value:item.value};});}
function qpbDrawBarChart(selector,values,kind){var host=document.querySelector(selector);if(!host)return;if(!values.length){host.innerHTML="<p class='chart-empty'>기록 없음 · 해당 없음</p>";return;}host.innerHTML="";var width=720,height=280,left=76,right=18,top=24,bottom=62,yTitle=kind==="평균 피도"?"평균 피도 (%)":"출현 횟수 (회)",max=Math.max.apply(null,values.map(function(v){return Number(v.value)||0;}))||1,svg=d3.select(selector).append("svg").attr("viewBox","0 0 "+width+" "+height).attr("preserveAspectRatio","xMidYMid meet").attr("role","img").attr("aria-label",kind+" 차트 · 가로축은 종명 (국명/학명) · 세로축은 "+yTitle),x=d3.scaleBand().domain(values.map(function(v,index){return String(index);})).range([left,width-right]).padding(.22),y=d3.scaleLinear().domain([0,max]).nice().range([height-bottom,top]);svg.append("line").attr("x1",left).attr("x2",width-right).attr("y1",height-bottom).attr("y2",height-bottom).attr("stroke","currentColor").attr("opacity",.45);svg.append("line").attr("x1",left).attr("x2",left).attr("y1",top).attr("y2",height-bottom).attr("stroke","currentColor").attr("opacity",.45);values.forEach(function(item,index){var value=Number(item.value)||0,label=String(item.name||"미동정"),bar=svg.append("rect").attr("x",x(String(index))).attr("y",y(value)).attr("width",Math.max(8,x.bandwidth())).attr("height",Math.max(0,y(0)-y(value))).attr("fill","#2f8f70").attr("rx",3).attr("tabindex","0");bar.node().setAttribute("aria-label",label+": "+value);bar.node().addEventListener("focus",function(){document.getElementById("limitations").textContent=kind+" 선택: "+label+" = "+value;});svg.append("text").attr("x",x(String(index))+x.bandwidth()/2).attr("y",height-bottom+20).attr("text-anchor","middle").attr("font-size","12").attr("fill","currentColor").text(label);svg.append("text").attr("x",x(String(index))+x.bandwidth()/2).attr("y",y(value)-6).attr("text-anchor","middle").attr("font-size","12").attr("font-weight","700").attr("fill","currentColor").text(value);});svg.append("text").attr("class","chart-y-title").attr("transform","translate(18,"+((top+height-bottom)/2)+") rotate(-90)").attr("text-anchor","middle").attr("font-size","12").attr("fill","currentColor").text(yTitle);svg.append("text").attr("x",(left+width-right)/2).attr("y",height-10).attr("text-anchor","middle").attr("font-size","12").attr("fill","currentColor").text("종명 (국명/학명)");}
...
function renderSummaries(){
  var labels={site:"조사지",survey:"조사",plot:"조사구"},out="";
  Object.keys(labels).forEach(function(name){
    var table=(data.tables||[]).filter(function(item){return item.name===name;})[0];
    if(!table)return;
    var records=table.records||[],fields=(table.fields||[]).filter(function(field){
      return field.name!=="geom"&&field.name!=="geometry"&&qpbNormalColumn({key:field.name,source_field:field.source_field||field.name});
    });
    out+="<details><summary aria-expanded='false'>"+labels[name]+" 기록 수: "+records.length+"</summary>";
    if(!records.length)out+="<p class='empty'>등록된 "+labels[name]+" 기록이 없습니다.</p>";
    else if(!fields.length)out+="<p class='empty'>기록은 있으나 표시할 속성이 없습니다.</p>";
    else{
      out+="<div class='table-wrap'><table><thead><tr>"+fields.map(function(field){return "<th>"+esc(qpbKoreanFieldLabel(name,field.name,field.label||field.name))+"</th>";}).join("")+"</tr></thead><tbody>";
      records.forEach(function(record){
        out+="<tr>"+fields.map(function(field){var value=(record.attrs||{})[field.name];return "<td>"+esc(value===null||value===undefined||value===""?"—":value)+"</td>";}).join("")+"</tr>";
      });
      out+="</tbody></table></div>";
    }
    out+="</details>";
