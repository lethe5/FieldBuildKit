...
function create(deps) {
    var state = {message: "", busy: false, expanded: false, listed: [], candidate: null, snapshot: null,
        ...
        settings: {server_url: "https://api.openrouteservice.org", optimizer_url: "", backend: "ors-vroom", profile: "driving-car", key: "", timeout_ms: 30000, max_road_offset_m: 1000, objective: "time"}};
    ...
    function reload() {
        try { state.snapshot = deps.repository.load(deps.io, deps.base); Object.assign(state.settings, state.snapshot.data.settings); state.settings.key = ""; if(state.snapshot.data.settings.mapping) state.mapping=clone(state.snapshot.data.settings.mapping); if(active() && active().mapping) state.mapping=clone(active().mapping); state.message = state.snapshot.recovered ? "손상된 저장본을 발견하여 정상 사본을 불러왔습니다." : ""; notify(); }
        catch(e) { error(e); throw e; }
    }
    function settings(values) {
        var merged = Object.assign({}, state.settings, values);
        if (!/^https?:\/\//.test(merged.server_url) || !Number.isInteger(merged.timeout_ms) || merged.timeout_ms <= 0 || !isFinite(merged.max_road_offset_m) || merged.max_road_offset_m < 0) throw new Error("서버 주소, 제한시간, 도로 이격거리를 확인하세요.");
