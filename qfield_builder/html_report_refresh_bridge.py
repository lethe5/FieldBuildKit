...
_FUNCTIONS = (
    ...
    "qpbBytes", "qpbGpkgEnvelopeGeoJson", "qpbTransformGeoJson", "qpbDecodeGpkgGeometry", "qpbCollectGpkgSpatialRows",
    ...
    "qpbKoreanFieldLabel", "qpbJoinedColumns", "qpbMakeJoinedRow", "qpbBuildJoinedRowsRuntime", "qpbBuildJoinedRows",
...
var QfLayerUtils={createFeatureIterator:function(layer){var index=0;return {hasNext:function(){return index<layer.__features.length;},next:function(){var feature=layer.__features[index++];layer.__setCurrent(feature);return feature;},close:function(){}};}};
var QfFeatureUtils={createBlankFeature:function(){return null;},attributeIsNull:function(value){return value===null||value===undefined;}};
...
if(direct.available===true){qgisProject.executeSql=function(sql){var text=String(sql||""),tables=direct.tables||[];if(/FROM gpkg_contents/i.test(text))return tables.map(function(table){return {table_name:table.name,data_type:"features"};});if(/FROM gpkg_geometry_columns/i.test(text))return tables.map(function(table){return {table_name:table.name,column_name:table.geometry_column,geometry_type_name:table.geometry_type,srs_id:table.srs_id};});if(/FROM gpkg_spatial_ref_sys/i.test(text))return tables.map(function(table){return {srs_id:table.srs_id,definition:String(table.srs_id)};});var pragma=text.match(/^PRAGMA table_info\("([^"]+)"\)/i),select=text.match(/^SELECT [\s\S]+? FROM "([^"]+)"/i);if(pragma){var table=tables.filter(function(item){return item.name===pragma[1];})[0];return table?(table.columns||[]).map(function(name){return {name:name};}):[];}if(select){var selected=tables.filter(function(item){return item.name===select[1];})[0];return selected?(selected.rows||[]).map(function(raw){var row=Object.assign({},raw),shape=raw[selected.geometry_column];if(text.indexOf("__qpb_geometry_bytes")>=0){row.__qpb_geometry_bytes=shape===null||shape===undefined?null:JSON.stringify(shape).length;row.__qpb_geometry_prefix=shape;row.__qpb_geometry_full=shape;}return row;}):[];}return [];};qgisProject.qpbReportRowCrs=function(table,id,source){var overrides=direct.row_crs_overrides||{};return Object.prototype.hasOwnProperty.call(overrides,String(id))?overrides[String(id)]:source;};}
