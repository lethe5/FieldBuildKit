...
| The cycle-3 journal still emitted every result mutation from `ObservedState.__setitem__/update`, attached all prior events as parents, represented cold start with UUID/loaded flags, and returned constant toggle counts/change paths/parameter-overlaid visibility. AC053 remained capped at three fixed accessibility objects. | D-SRP-049–059; FR-SRP-049–056; AC-SRP-052/053/056/058 | Hook registry and event payloads identify concrete production callbacks and predeclared projections; AST/tamper guards reject result-mutation journaling and wholesale causal parents. Cold start uses a distinct subprocess/PID plus reopened generated-project hash. Storage/QML callback payloads determine toggle IO, feedback and visibility. Two five-visit documents use dynamic Repeater/QAccessible delegates, with exact-zero after index 3 and no fixed numbered IDs. |
...
| Metric-source accessibility used a fixed source literal, walking surfaces did not prove mapped and lower-bound quantities stayed separate, and notice order came from QML object-tree DFS. | D-SRP-054/055; FR-SRP-050/052; AC-SRP-053/054 | Two changed all-mode documents drive every visit's site/mode/source/왕복/distance/time accessibility from repository readback, including exact-zero. Separate values bind mapped provider distance/time and straight-line lower-bound to distinct visible/QAccessible objects without exact-total summing. Notice precedes calculate in real `QAccessible` parent/child traversal. |
...
| AC-SRP-058 | `ac058_route_line_toggle_is_one_atomic_settings_only_write_and_persists` (test-derived storage diff/commit/readback and actual QML visibility); `ac058_preview_and_legend_are_write_free` (installed observer logs); `ac058_toggle_failure_rolls_back_and_reports_actionable_feedback` (storage/QML callback evidence) | M03/M19 native QField persistence/rendering remains NOT RUN |
