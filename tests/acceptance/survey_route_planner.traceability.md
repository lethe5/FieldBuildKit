...
| Empty provenance lists, filtered request logs, case-derived preflight, serial capture hashes, one-time renderer hashes, mirrored accessibility fields and hardcoded zero-activity arrays could pass without observing production. | D-SRP-049–056; FR-SRP-047–053; AC-SRP-049–055 | Field-to-event lineage, unfiltered origin/failure transport, controller preflight, semantic action states, independent renderer rereads, runtime accessibility interfaces, actual viewport/theme and installed navigation request/write observers are mandatory. |
| Schema-3 route-level fields were checked, but visit identity/provenance remained unchecked. | D-SRP-057; FR-SRP-054; AC-SRP-056 | Active/inactive × all missing/blank/mismatch/unknown/disallowed-pair corruptions reject load and recovery atomically; all three allowed pairs exact-roundtrip; schema 1/2 remain read-only. |
...
| AC-SRP-052 | `ac052_schema3_exact_roundtrip_recovery_move_and_completion_are_offline` (mapped/fallback); `ac052_every_schema3_route_rejects_required_field_omission` (five required fields omitted from inactive second production-saved route); `ac052_ac056_legacy_load_and_future_rejection_preserve_bytes` (schema 1/2 plus future) | M18; target QField native file/recovery behavior remains NOT RUN |
| AC-SRP-053 | `ac053_mixed_route_visual_accessibility_proxy_is_distinct_and_passive` (2 widths × 2 themes; actual viewport/theme, semantic state transitions, renderer rereads and separate runtime accessibility objects) | M19 authoritative native render/grayscale/screen-reader gate; NOT RUN |
| AC-SRP-054 | `ac054_exact_stage_sequence_privacy_and_no_incidental_writes` (actual visual/accessibility notice order and explicit click observation); `ac054_lifecycle_after_origin_validation_starts_no_later_request_or_write` (cancel/project close); `ac054_each_stage_failure_stops_all_later_requests_and_writes` (7 stages); `ac054_status_actions_remain_redacted_and_nonretrying` (401/403/404/429/5xx/timeout/radius); AC049/050/051 fail-fast matrices | M18; redacted live-provider confirmation remains NOT RUN |
...
| AC-SRP-056 | `ac056_every_active_and_inactive_visit_corruption_rejects_whole_document` (2 route states × 2 visit positions × 15 corruptions, each load+recovery); `ac056_three_allowed_visit_provenance_pairs_roundtrip_exactly` (3); retained schema-1/2 compatibility in `ac052_ac056_legacy_load_and_future_rejection_preserve_bytes` | M18 native file/recovery remains NOT RUN |
...
| D-SRP-053; FR-SRP-051 | AC-SRP-052 schema-3 totals/provenance/recovery plus every-route required-field omission rejection and schema-1/2 compatibility tests; M18 |
| D-SRP-054; FR-SRP-050 | AC-SRP-053 generated presentation/accessibility proxy with real theme/action/source observations; M19 authoritative device gate |
| D-SRP-055; FR-SRP-052 | AC-SRP-054 notice/request-shape/privacy/no-write test, post-origin cancel/project-close quiescence and all injected failure boundaries; M18 |
...
| D-SRP-057; FR-SRP-054 | AC-SRP-056 active/inactive visit corruption rejection, exact three-pair roundtrip and schema-1/2 compatibility; M18 |
