...
| Empty provenance lists, operation tables, filtered request logs, case-derived preflight, serial capture hashes, one-time renderer hashes, mirrored accessibility fields and hardcoded zero-activity arrays could pass without observing production; the first journal contract could still be synthesized after result assembly. | D-SRP-049–056; FR-SRP-047–053; AC-SRP-049–055 | Test-computed lineage comes from callback events closed and detached-sealed before result materialization. Timing/seal/read-twice checks, explicit causal links, boundary-rooted raw fields and pre-existing event/callback bindings reject `result_source`, `bind_result`, result replay/field inference and implicit predecessor causality. Unfiltered transport, controller preflight, semantic states, renderer rereads, runtime accessibility and installed navigation observers remain mandatory. |
...
| Metric-source accessibility used a fixed source literal, walking surfaces did not prove mapped and lower-bound quantities stayed separate, and notice order came from QML object-tree DFS. | D-SRP-054/055; FR-SRP-050/052; AC-SRP-053/054 | Three stored source variants drive one QML accessible metric-source property from repository readback. Two changed mixed-route values dynamically bind mapped provider distance/time and straight-line lower-bound to distinct visible and QAccessible objects without summing or exact-total labeling. Notice precedes calculate in real `QAccessible` parent/child traversal. |
...
| D-SRP-054; FR-SRP-050 | AC-SRP-053 generated presentation/accessibility proxy with real theme/action/source observations, dynamic stored `visit.metric_source` binding, and separate dynamic mapped provider vs straight-line-lower-bound visible/QAccessible quantities; M19 authoritative device gate |
