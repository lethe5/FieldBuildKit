...
> acceptance approvals remain preserved. The current correction strengthens AC-SRP-049–055
> observation integrity and adds approved AC-SRP-056 visit-validation coverage.
...
| AC028/036 treated route-line toggle as persistent/revision-changing. | D-SRP-055; FR-SRP-052/AC-SRP-054 | In-session only, zero provider/route/settings write and zero revision/staleness; reconstructed session returns default-on. |
...
| Empty provenance lists, operation tables, filtered request logs, case-derived preflight, serial capture hashes, one-time renderer hashes, mirrored accessibility fields and hardcoded zero-activity arrays could pass without observing production. | D-SRP-049–056; FR-SRP-047–053; AC-SRP-049–055 | Test-computed lineage from a hash-chained immutable boundary-event journal, unfiltered origin/failure transport, controller preflight, semantic action states, independent renderer rereads, runtime accessibility interfaces, actual viewport/theme and installed navigation request/write observers are mandatory. |
| Schema-3 visit tests checked pair labels but not their approved distance meanings; legacy upgrade checked neither valid schema 2 replacement nor failure atomicity. | D-SRP-027/051/053/057; FR-SRP-049/051/054; AC-SRP-052/056 | Active/inactive × identity/pair/distance corruptions reject load and recovery atomically; mapped/provider, exact-zero and unmapped/geodesic semantics exact-roundtrip; explicit valid schema1/2 upgrade replaces the same identity once and preserves bytes/route/last-good on failure. |
| Metric-source accessibility used a fixed source literal and notice order came from QML object-tree DFS. | D-SRP-054/055; FR-SRP-050/052; AC-SRP-053/054 | Three valid stored source variants must drive the same production QML accessible property exactly from repository-read `visit.metric_source`; notice precedes calculate in real `QAccessible` parent/child traversal. |

Current retry-1 DRAFT verification: design verifier passed; focused AC049–056 collection found
**167/525** cases; a loopback-enabled 11-case reviewer-finding slice produced the expected
**11 failed, 514 deselected** because the immutable event journal/dynamic accessibility operation is
not implemented; one explicit unmapped-offset corruption case also failed as unsupported.
...
| AC-SRP-028 | `ac028_metric_formatting_and_bottom_bar` (4 ceil-minute edges), `ac028_route_line_toggle_scope_persistence_move_and_zero_api` (passive in-session toggle, default-on after reconstruction, zero write/revision), `ac027_ac028_progression_survives_restart_recovery_offline_move` | M03/M04; native overlay visibility |
...
| AC-SRP-052 | `ac052_schema3_exact_roundtrip_recovery_move_and_completion_are_offline` (mapped/fallback); `ac052_every_schema3_route_rejects_required_field_omission` (five required fields omitted from inactive second production-saved route); `ac052_ac056_legacy_load_and_future_rejection_preserve_bytes` (schema 1/2 plus future); `ac052_selected_schema1_and_schema2_route_upgrade_atomically_replaces_identity` (both valid legacy schemas); `ac052_failed_schema1_and_schema2_upgrade_preserves_selected_route_and_bytes` (calculation/commit failure) | M18; target QField native file/recovery behavior remains NOT RUN |
| AC-SRP-053 | `ac053_mixed_route_visual_accessibility_proxy_is_distinct_and_passive` (2 widths × 2 themes; actual viewport/theme, semantic state transitions, renderer rereads and separate runtime accessibility objects); `ac053_accessible_metric_source_tracks_runtime_visit_value_without_source_oracle` (three valid stored variants, no source literal oracle) | M19 authoritative native render/grayscale/screen-reader gate; NOT RUN |
...
| AC-SRP-055 | `ac055_ios_uses_exact_apple_maps_once_without_any_fallback` (true/false/exception); `ac055_navigation_coordinates_are_canonical_and_android_contract_is_unchanged` (4 boundaries); `ac055_invalid_navigation_coordinate_never_dispatches_or_mutates` (7 invalid inputs); retained Android-only AC039 tests; all use installed routing/storage observers | M20 iOS and M21 Android handoff; NOT RUN |
| AC-SRP-056 | `ac056_every_active_and_inactive_visit_corruption_rejects_whole_document` (2 route states × 2 visit positions × 19 identity/pair/distance corruptions, each load+recovery); `ac056_three_allowed_visit_provenance_pairs_roundtrip_exactly` (3 with provider/exact-zero/geodesic semantic oracles); retained schema-1/2 compatibility in `ac052_ac056_legacy_load_and_future_rejection_preserve_bytes` | M18 native file/recovery remains NOT RUN |
...
| D-SRP-053; FR-SRP-051 | AC-SRP-052 schema-3 totals/provenance/recovery, every-route required-field omission rejection, schema-1/2 read-only compatibility and explicit same-identity atomic schema-3 replacement/failure preservation; M18 |
| D-SRP-054; FR-SRP-050 | AC-SRP-053 generated presentation/accessibility proxy with real theme/action/source observations and dynamic stored `visit.metric_source` binding; M19 authoritative device gate |
...
| D-SRP-057; FR-SRP-054 | AC-SRP-056 active/inactive identity/pair/distance corruption rejection, provider/exact-zero/geodesic three-pair semantic roundtrip and schema-1/2 compatibility; M18 |
