# Survey route acceptance harness — DRAFT evidence-integrity/schema-3 correction
...
`production_provenance.evidence_integrity` contains nonempty `observation_events` with unique IDs,
an exhaustive `field_origins` entry for every criterion-critical field named by the executable
contract, and empty `hardcoded_result_fields`/`unattributed_result_fields`. Each origin event names
the observed fields, one of the approved sources (`production_call`, `runtime_property`,
`captured_transport`, `captured_storage`, `loaded_artifact`, `accessibility_interface`,
`source_layer_reread`, `signal_delivery`) and `copied_from_case=false`. Merely returning empty
postprocessing/case-copy lists, source hashes, or a declared operation name is insufficient.
