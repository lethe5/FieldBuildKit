...
| `builder_step7_route_credentials` | Drive actual Step 7 widgets and normal build flow for blank, key-without-consent, key-with-consent and remember-only states. Traverse the actual Qt widget tree and parent layouts, returning stable widget identities and layout paths in observed reading order. Traverse `QWidget.nextInFocusChain` for the focusable key/consent/remember controls. Query each widget with `QAccessible.queryAccessibleInterface`; when an interface exists return its actual name/role/source, and when unavailable return `available=false` plus a diagnostic without synthetic true/name/role values. Return the exact copy, masking/wrapping, build result, variables, artifact paths and redacted diagnostics. Only the consented state may contain the synthetic key, exactly once in the generated QGS project variable. |
