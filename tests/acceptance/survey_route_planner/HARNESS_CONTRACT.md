...
`boundary_event_journal`. The journal is UTF-8 JSONL closed before result assembly; provenance gives
its path, run ID and SHA-256. Each hash-chained record has a unique `event_id`, zero-based `sequence`,
`previous_event_sha256`, canonical `event_sha256`, earlier `parent_event_ids`, an observer naming the
real production source and boundary, one approved source (`production_call`, `controller_state`,
`captured_transport`, `captured_storage`, `loaded_artifact`, `accessibility_interface`,
`source_layer_reread`, `signal_delivery`, `navigation_launcher`, `render_observation`), and the raw
observed production field paths/values. Per-event `output_bindings` map returned top-level names to
different dotted raw field paths in that same event; the test resolves those bindings itself.
Production-call counts are computed from these records.

The executable test reads and hashes the journal itself, validates the chain and parent order, and
computes field lineage from its raw records. Every mixed-operation output field consumed by
