# Survey route acceptance harness — DRAFT production-path/supersession correction
...
> evidence from AC-SRP-049–055 and reconciles retained executable expectations with the approved
...
For AC-SRP-049–055, dispatching to `tests/unit/survey_route_mixed_driver.js` or any equivalent
...
than declared from the operation name. It contains `operation`, `result_origin=production_observation`,
empty `adapter_postprocessed_fields`, `case_copied_result_fields` and
`fixture_expected_values_used_as_results`, the actual driver path, counted `production_calls`, and
`production_sources` entries with repository-relative path and SHA-256 for each executed production
source. Generated-QML operations additionally return `project_dir` and `generated_qml` with the
loaded project-relative file path/hash. The executable test independently hashes those files. A
test/unit mixed driver, copied source hash, or call list without the observed call events is not evidence.
...
| Operation | Required driving steps and returned evidence |
| --- | --- |
| `provider_http_failure` | Through the generated QML calculate control and production controller/backend, create and independently reopen nonempty saved/candidate baselines, then inject one HTTP or statusless transport result at the real transport stage. Return the production minimal record, live message-label diagnostic, classification/action, intercepted request, raw-body-retention flag and independently observed state/write/retry snapshots. Production first attempts JSON parsing for a syntactically valid body even when content type is missing or misleading; content type does not suppress safe scalar code/message extraction. `status_0`/network records have `http_status=null`, never synthesize HTTP 0, and expose an actionable connection check. Sanitization is performed only by production. Status 200 is a bounded negative oracle: its body must never enter error UI. |
| `mixed_route_calculate` | Drive notice acknowledgement and the explicit calculate control. Execute production preflight/origin validation, one ordered batched `driving-car` snap, per-site walking validation, matrix, optimizer, driving directions and candidate validation. Inject only documented provider responses/faults. Return exact request/stage traces, snap/radius/origin workaround observations, request payload field sets, before/after source/state/storage snapshots and the actual candidate. The exact-zero case may legitimately have `source_coordinate == access_coordinate`; only a nonzero snapped source is forbidden from downstream vehicle payloads, which must still contain access coordinates exclusively. `batch-limit` is rejected before transport. `null`, radius rejection/cap, origin failure and transient/malformed walking faults stop all downstream work. Only an explicit no-foot-path response may continue as unmapped lower-bound, and its first save attempt is blocked until the real acknowledgement is made. When cancel or project close is delivered through production immediately after origin validation, no access-snap or later request starts and source/completion/settings/route storage is not written. |
| `mixed_route_roundtrip` | Create a mapped or one-fallback candidate through generated QML and production controller/backend, explicitly save schema 3 through the real save control/repository, independently reopen committed bytes, then perform restart, last-good recovery, offline open, physical folder relocation and complete→uncheck through real controls. Echoing the candidate as saved/reloaded, constructing a schema wrapper in the harness, or calculating remaining counts in the harness is forbidden. |
| `mixed_route_compatibility` | Materialize exact supplied schema 1/2 or future-schema bytes, load them offline through production `repository.js`, and independently reread the same file after the action. For schema-3 omission cases, create two routes through real calculate/save actions, fault only the committed second/inactive route by omitting one required field at the storage boundary, then load through `repository.js`; return production save events and prove whole-document rejection, unchanged bytes, no repair/write/request. A harness-only input-dictionary comparison or `schema > 3` branch is forbidden. |
| `mixed_route_presentation` | Commit and independently reload a real schema-3 route, then load the generated `RoutePanel.qml` object tree/map at supplied width/theme. Observe real overlay object IDs/canvas stroke settings, all three patterns, contrasting casing, non-color cues, legends, warning marker, preview/detail/bottom-summary accessibility values and passive toggle/completion state. `presentation_provenance` records the loaded object-tree/map source, actual effective host theme/object, window-delivered preview/toggle/complete/uncheck actions with distinct before/after capture IDs, and loaded source-layer renderer hashes/identity before and after. Constant line-class/field/theme/action/source dictionaries are forbidden. It remains a generated/headless proxy only. |
| `platform_map_dispatch` | Save and independently reload an active route through production repository/controller, locate and invoke the loaded generated QML control whose exact label is `다음 지점 지도 안내`, and capture the controller→navigation call plus every real `Qt.openUrlExternally` call/result/exception, state, writes and revisions. iOS permits one Apple Maps HTTPS call and no fallback; Android retains NAVER intent/Google Play fallback. The harness never returns a production-computed expected URL for a circular test comparison. |
