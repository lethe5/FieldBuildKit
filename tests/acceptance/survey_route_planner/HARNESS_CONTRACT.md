...
| Operation | Required driving steps and returned evidence |
| --- | --- |
| `provider_http_failure` | Through the generated QML calculate control and production controller/backend, create and independently reopen nonempty saved/candidate baselines, then inject one HTTP result at the real transport stage. Return the production minimal record, live message-label diagnostic, classification, intercepted request, raw-body-retention flag and independently observed state/write/retry snapshots. Sanitization is performed only by production. Status 200 is a bounded negative oracle: its body must never enter error UI. |
...
| `mixed_route_compatibility` | Materialize exact supplied schema 1/2 or future-schema bytes, load them offline through production `repository.js`, and independently reread the same file after the action. A harness-only input-dictionary comparison or `schema > 3` branch is forbidden. |
| `mixed_route_presentation` | Commit and independently reload a real schema-3 route, then load the generated `RoutePanel.qml` object tree/map at supplied width/theme. Observe real overlay objects/canvas stroke settings, patterns/casing/legend/warning marker, preview/detail/bottom-summary accessibility values and passive toggle/completion state. Constant line-class/field dictionaries are forbidden. It remains a generated/headless proxy only. |
