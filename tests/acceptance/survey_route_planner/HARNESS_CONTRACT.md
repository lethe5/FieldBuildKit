...
| `platform_naver_dispatch` | Drive the one visible Naver action with injected platform/host capability and launcher Boolean results. Capture every real `Qt.openUrlExternally` call in order. Android supported-native uses the exact package-bound `intent://navigation?...#Intent;scheme=nmap;action=android.intent.action.VIEW;category=android.intent.category.BROWSABLE;package=com.nhn.android.nmap;end`; iOS uses `nmap://navigation?...`. Official fallbacks are exactly `market://details?id=com.nhn.android.nmap` and `http://itunes.apple.com/app/id311867728?mt=8`. With no documented navigation-web fixture, no web URL is permitted. Return only OS-request/install-page/failure status and false app/destination/navigation claims. |
...
| `builder_step7_route_credentials` | Drive actual Step 7 widgets and normal build flow for blank-key-with-both-checked, non-blank-neither, consent-only, remember-only and both states. Traverse the actual Qt widget tree and parent layouts, returning stable widget identities and layout paths in observed reading order. Traverse `QWidget.nextInFocusChain` for the focusable key/consent/remember controls. Query each widget with `QAccessible.queryAccessibleInterface`; when an interface exists return its actual name/role/source, and when unavailable return `available=false` plus a diagnostic without synthetic true/name/role values. Return the exact copy, masking/wrapping, build result, variables, artifact paths and redacted diagnostics. Independently reopen the isolated encrypted desktop store and return only whether a retained value exists, its encrypted-store provenance and whether plaintext exists at rest; never return the key. Only non-blank consent may create exactly one QGS project-variable occurrence, and only non-blank remember may create encrypted desktop retention. |

### 2026-09-17 iOS theme/spacing/default-name and QPB follow-up operations — APPROVED TEST DESIGN (approval 2026-09-18)

These operations are acceptance seams, not shortcuts around the approved evidence boundary. Render
observations come from production-generated QML or generated project artifacts. The driver must not
copy expected values from the case, calculate a pre-baked pass flag, claim an iOS keyboard opened, or
claim a target-QField screenshot was captured.

| Operation | Required driving steps and returned evidence |
| --- | --- |
| `route_panel_theme_contrast_proxy` | Load the generated route panel in a real QML runtime at 320 px for the requested light/dark appearance and state. Render a screenshot, identify the actual semantic text/control regions, and sample foreground/background pixels away from antialiased edges. Return raw RGB samples, semantic role, text/non-text kind, threshold and screenshot provenance; the test independently computes WCAG contrast. State-specific non-color cues must be observed from the rendered control, not inferred from a color literal. Keep `claims.ios_device_contrast_pass` absent/false. |
| `route_panel_theme_switch_proxy` | Create one real candidate and populated panel, focus a control and set a nonzero scroll position. Drive light→dark→light through the runtime theme boundary. Snapshot values, focused object ID, candidate, route and scroll position after each change, plus complete routing-request and persistence-write captures. No snapshot may be synthesized from the initial case. |
| `route_panel_header_spacing_proxy` | Load the generated expanded panel at the requested width, supported text scale and state. Render it and derive the header's complete painted bottom (including shadow/focus treatment), first `조사지` label painted top, label/notch/outline clipping and horizontal overflow from image edges or scenegraph painted bounds. Return device-pixel ratio and actual hit rectangles, then dispatch separate label, control and header taps through the window hit-test path. Keep `claims.ios_device_spacing_pass` absent/false. |
| `route_name_local_date_lifecycle` | Use an injected device clock/timezone only at the platform-clock boundary and drive two explicit successful calculations through production. Observe the first default, actual tap/caret/select/delete/IME event sequence, user-edited control text, collapse/reopen, theme change, refresh, save retry, failed calculation, trimmed save/reopen, and the second successful calculation. Capture candidate identity and requests for every action. Locale must not affect the fixed format. Keep `claims.ios_soft_keyboard_opened` absent/false. |
| `generated_site_tabler_exclusion` | Build through the normal project generator with a canonical Point/MultiPoint `site`, every eligible non-site semantic point layer present in the fixture, and the requested icon mode. Parse the generated QGS before and after alternate display-name changes and optional whole-folder relocation/reopen. Return canonical identity, symbol-layer types and relative SVG references for every eligible layer, plus independent before/after geometry, attribute, label, relation and route-overlay snapshots. Also observe polygon/line site and Type 4 community preservation. No-icon and failed-fetch runs must use the existing minimalist fallback and contain no selected SVG reference. |
