# Survey route acceptance harness — DRAFT reconciliation, 2026-09-14

Specification checkpoint: `3b08820` (approved Category B clarification). The earlier adapter
...
| `configured_calculate` | Seed a saved active route, apply each setting through the production project/session settings path and press calculate. Dispatch registered `ors-vroom` and return `transport_settings` decoded from the actual provider dispatch and intercepted HTTP requests, plus independently decoded `persisted_settings`, diagnostics and saved/active before/after. The calculation remains an unsaved preview. Omitted road offset defaults to 1000 m. An unknown provider ID must fail before registry transport dispatch, issue no request, and preserve the saved/active route. The key may reach transport only; it must not enter persisted settings, project files, logs or errors. |
...
O-SRP-003 and O-SRP-005 are resolved by approved checkpoint `3b08820`; centroid, Boolean
