...
> `ObservedState`, `__setitem__`/`update` journaling and any result-mutation callback are rejected.
> Hook registration names the actual callback implementation and its predeclared projection
> paths; the callback-owned raw payload is appended before a pure post-seal projection. Causal
> parents name only originating events, never every earlier production event. Cold start crosses a
> real subprocess/PID boundary and reopens the generated project. Toggle counts, changed paths,
> feedback and line visibility are derived from storage/QML callback observations. AC053 uses at
> least five dynamically repeated accessibility delegates, with exact-zero beyond index 3.
...
Each registered hook records its `hook_id`, actual `implementation_path` and SHA-256, concrete
`implementation_symbol` and projection paths declared before execution; the test independently
...
`callback_owner`, the detached callback-owned `callback_payload`, and
`event_appended_at_monotonic_ns` before aggregation. It has one approved
...
The hash-chain predecessor is never an implicit causal parent. `originating_event_ids` equals the
explicit parent list, and a generic relation to every prior production event is invalid. Each
...
are equally forbidden. Result construction is a pure post-seal projection of registered callback
payload paths. Production-call counts are computed from these records.
