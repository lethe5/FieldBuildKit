...
> five delegates through their visual parent's `itemAt`, then reads each `visitValue` and
...
hash, flush count and final callback-finish time; the executable test reads both files twice and
...
not a shared superset: every event's payload equals its concrete hook schema. The test independently
...
explicit parent list passed from the triggering callback; latest-event-by-source lookup and a
...
capture may inspect the completed result. `capture_outputs`, `dict(result)`, a synthetic
...
Result construction is a pure post-seal projection of registered callback payload paths, including
any returned `captured_request`. Production-call counts are computed from these records.
...
declaration loophole, every returned top-level output except `production_provenance`—must have an
